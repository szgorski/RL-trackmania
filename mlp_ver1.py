...
device_trainer = 'cuda:0'  # if cfg.CUDA_TRAINING else 'cpu'
...
class SquashedGaussianMLPActor(ActorModule):
    ...
    def forward(self, obs, test=False, with_logprob=True):
        # obs_seq = torch.stack([torch.from_numpy(o) for o in obs])
        # print(obs_seq[0])
        net_out = self.net(torch.tensor(obs[1]))
        mu = self.mu_layer(net_out)
        log_std = self.log_std_layer(net_out)
        log_std = torch.clamp(log_std, LOG_STD_MIN, LOG_STD_MAX)
        std = torch.exp(log_std)
...
class SACTrainingAgent(TrainingAgent):
    ...
    def __init__(self,
                 ...
                 device="cuda:0",
                 model_cls=MLPActorCritic,  # An actor-critic module, encapsulating our ActorModule
    ...
    def train(self, batch):
        ...
        pi, logp_pi = self.model.actor.forward(
            obs=o, test=False, with_logprob=True)
        ...
        q1_pi = self.model.q1.forward(o, pi)
        q2_pi = self.model.q2.forward(o, pi)
...
training_agent_cls = partial(SACTrainingAgent,  # SACTrainingAgent,
                             model_cls=MLPActorCritic,  # VanillaCNNActorCritic,
                             gamma=0.99,
                             polyak=0.995,
                             alpha=0.02,
                             lr_actor=0.000005,
...
if __name__ == "__main__":
    ...
    if args.trainer:
        ...
        rw = RolloutWorker(env_cls=env_cls,
                           actor_module_cls=SquashedGaussianMLPActor,  # MyActorModule
