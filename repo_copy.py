...
from tmrl.actor import TorchActorModule, ActorModule
...
# from tmrl.custom.custom_models import RNNActorCritic, SquashedGaussianRNNActor
...
class SquashedGaussianRNNActor(nn.Module, ActorModule):
...
class MyREDQSACAgent(TrainingAgent):
    def __init__(self,
                 observation_space=None,  # Gymnasium observation space (required argument here for your convenience)
                 action_space=None,  # Gymnasium action space (required argument here for your convenience)
                 device=None,  # Device our TrainingAgent should use for training (required argument)

                 model_cls=RNNActorCritic,  # An actor-critic module, encapsulating our ActorModule
                 gamma=0.99,  # Discount factor
                 polyak=0.995,  # Exponential averaging factor for the target critic
                 alpha=0.2,  # Value of the entropy coefficient
                 lr_actor=1e-3,  # Learning rate for the actor
        ...
        self.q_optimizer = Adam(self.q_params, lr=self.lr_critic)
        self.alpha_t = torch.tensor(float(self.alpha)).to(self.device)
        self.lr_entropy: float = 1e-3  # entropy autotuning
        self.learn_entropy_coef: bool = True
        self.target_entropy: float = None  # if None, the target entropy is set automatically
        self.n: int = 10  # number of REDQ parallel Q networks
        self.m: int = 2  # number of REDQ randomly sampled target networks
        self.q_updates_per_policy_update: int = 1  # in REDQ, this is the "UTD ratio" (20), this interplays with lr_actor

        self.model_nograd = cached_property(lambda self: no_grad(copy_shared(self.model)))

    # observation_space: type = None
    # action_space: type = None
    # device: str = None  # device where the model will live (None for auto)
    # model_cls: type = RNNActorCritic
    # gamma: float = 0.99
    # polyak: float = 0.995
...
training_agent_cls = partial(MyREDQSACAgent,  # SACTrainingAgent,
                             model_cls=RNNActorCritic,  # VanillaCNNActorCritic,
                             gamma=0.99,
                             polyak=0.995,
                             alpha=0.02,
                             lr_actor=0.000005,
...
my_run_name = cfg.TMRL_CONFIG["RUN_NAME"]
...
if __name__ == "__main__":
    ...
    if args.trainer:
        ...
        rw = RolloutWorker(env_cls=env_cls,
                           actor_module_cls=SquashedGaussianRNNActor,  # MyActorModule
