...
class LidarMLP(nn.Module):
    def __init__(self, q_net):
        super(LidarMLP, self).__init__()
        # Adjust the sizes based on your requirements
        if q_net:
            self.mlp = mlp([size_no_critic, 4*19 - 7, 256], nn.ReLU)
        else:
            self.mlp = mlp([size_no_critic, 4*19 - 7, 256], nn.ReLU)
...
class VanillaMLP(nn.Module):
    def __init__(self, q_net):
        ...
        self.q_net = q_net
        self.lidar_mlp = LidarMLP(q_net)
        if self.q_net:
            self.mlp_input_features = self.lidar_mlp.mlp[-1](
                torch.zeros(1, 4*19)).size(1)
        else:
            self.mlp_input_features = self.lidar_mlp.mlp[-1](
                torch.zeros(1, 4*19)).size(1)
        # MLP layers:
        ...
        self.mlp = mlp([self.mlp_input_features], nn.ReLU)
    ...
    def forward(self, x):
        ...
        if self.q_net:
            ...
            speed, lidar_signals, act1, act2, act = x
        else:
            # For the policy, the next action (act) is what we are computing, so we don't have it:
            speed, lidar_signals, act1, act2 = x

        lidar_output = self.lidar_mlp(lidar_signals)
        if self.q_net:
            combined_features = torch.cat(
                (speed, lidar_output, act1, act2, act), dim=1)
        else:
            combined_features = torch.cat(
                (speed, lidar_output, act1, act2), dim=1)
...
class VanillaCNNQFunction(nn.Module):
    ...
    def __init__(self, observation_space, action_space):
        super().__init__()
        self.net = VanillaMLP(q_net=True)  # q_net is True for a critic module
